...
class ProcessorMixin:
    ...
    def _get_multimodal_processed_status(self, doc_status: Dict[str, Any]) -> bool:
        """
        Helper method to get multimodal_processed status from doc_status.
        Checks both metadata (new format) and top-level (legacy format) for backwards compatibility.

        Args:
            doc_status: Document status dictionary

        Returns:
            bool: True if multimodal processing is complete
        """
        if not doc_status:
            return False
        ...
        if "multimodal_processed" in metadata:
            return metadata["multimodal_processed"]

        # Fallback to top-level (legacy format for backwards compatibility)
        return doc_status.get("multimodal_processed", False)
        ...
        **kwargs,
        ...
        try:
            ...
            if ext in [".pdf"]:
                ...
                self.logger.info("Detected CSV file, parsing directly...")
                content_list = await asyncio.to_thread(
                    doc_parser.parse_csv,
                    csv_path=file_path,
                )
            else:
    ...
    async def _process_multimodal_content(
        ...
        try:
            ...
            if existing_doc_status:
                ...
                multimodal_processed = self._get_multimodal_processed_status(existing_doc_status)
    ...
    async def _mark_multimodal_processing_complete(self, doc_id: str):
        ...
        try:
            current_doc_status = await self.lightrag.doc_status.get_by_id(doc_id)
            if current_doc_status:
                # Store multimodal_processed in metadata to avoid DocProcessingStatus schema conflicts
    ...
    async def is_document_fully_processed(self, doc_id: str) -> bool:
        ...
        try:
            ...
            multimodal_processed = self._get_multimodal_processed_status(doc_status)
    ...
    async def process_content_list_direct(
        self,
        content_list: List[Dict[str, Any]],
        ...
        **kwargs,
        ...
        """
        Process a pre-parsed content list directly (for URL fetcher with extracted images)

        This method bypasses the document parsing stage and processes an already-parsed
        content list. This is useful for URL fetcher which extracts text and images separately.

        Args:
            content_list: Pre-parsed content list with text and image blocks
            source_identifier: Source identifier (e.g., URL or file path)
            doc_id: Optional document ID, if not provided will be generated from content
            enable_image_processing: Whether to process images with vision model
        ...
        try:
            ...
            self.logger.info(f"Processing content list directly from: {source_identifier}")
            self.logger.info(f"Content list contains {len(content_list)} blocks")
            ...
            try:
                existing_doc_status = await self.lightrag.doc_status.get_by_id(doc_id)
                if existing_doc_status and existing_doc_status.get("status") == DocStatus.PROCESSED.value:
                    self.logger.info(f"Document {doc_id} is already processed")
                    return {"success": True, "doc_id": doc_id, "message": "Already processed"}
            except Exception:
                pass  # Document doesn't exist, continue processing

            # Separate content into text and multimodal
            text_content, multimodal_content = separate_content(content_list)

            self.logger.info(f"Separated content: {len(text_content)} text blocks, {len(multimodal_content)} multimodal blocks")

            # Process text content first
            if text_content:
                await insert_text_content(
                    rag=self,
                    text_items=text_content,
                    doc_id=doc_id,
                    file_path=source_identifier
                )
                self.logger.info("Text content processing complete")

            # Process multimodal content if enabled and available
            if enable_image_processing and multimodal_content:
                self.logger.info(f"Processing {len(multimodal_content)} multimodal items...")
                await self._process_multimodal_content(
                    multimodal_items=multimodal_content,
                    file_path=source_identifier,
                    doc_id=doc_id
                )
                self.logger.info("Multimodal content processing complete")

            self.logger.info(f"Content list from {source_identifier} processing complete!")

            return {
                "success": True,
                "doc_id": doc_id,
                "text_blocks": len(text_content),
                "multimodal_blocks": len(multimodal_content),
                "message": "Processing complete"
            }

        except Exception as e:
            self.logger.error(f"Error processing content list: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
