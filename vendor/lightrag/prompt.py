...
3.  **Delimiter Usage Protocol:**
    *   The `{tuple_delimiter}` is a complete, atomic marker (the FULL string `{tuple_delimiter}`) and **must not be filled with content, truncated, or modified**. It serves strictly as a field separator.
    *   **CRITICAL**: Always output the COMPLETE delimiter `{tuple_delimiter}` - NEVER output partial delimiters like `<|` or `|>` alone.
    *   **Incorrect Example:** `entity{tuple_delimiter}Tokyo<|location|>Tokyo is the capital of Japan.`
    *   **Incorrect Example:** `relation{tuple_delimiter}United States<|N/A` (partial/truncated delimiter)
    *   **Correct Example:** `entity{tuple_delimiter}Tokyo{tuple_delimiter}location{tuple_delimiter}Tokyo is the capital of Japan.`
    *   **Correct Example:** `relation{tuple_delimiter}United States{tuple_delimiter}Canada{tuple_delimiter}border{tuple_delimiter}The two countries share a border.`
...
2.  **Field Count Validation**:
    ...
    *   Each relation line MUST have exactly 5 fields separated by `{tuple_delimiter}`
    *   NEVER output partial or incomplete lines
...
4.  **Completion Signal:** Output `{completion_delimiter}` as the final line after all relevant entities and relationships have been extracted and presented. This is MANDATORY.
...
5.  **Field Count Validation - CRITICAL**:
    *   Each entity line MUST have exactly 4 fields separated by the COMPLETE `{tuple_delimiter}`
    *   Each relation line MUST have exactly 5 fields separated by the COMPLETE `{tuple_delimiter}`
    *   NEVER output partial delimiters like `<|` without the full `{tuple_delimiter}`
    *   NEVER output incomplete lines
...
7.  **Completion Signal:** Output `{completion_delimiter}` as the final line after all relevant missing or corrected entities and relationships have been extracted and presented. This is MANDATORY.
