...
class EnhancedMarkdownConverter:
    ...
    def _sanitize_html_for_pdf(self, html: str) -> str:
        """
        Sanitize HTML content to be compatible with PDF generation.
        Removes unsupported attributes like 'style' from img tags.
        ...
        """
        def replace_style_with_attrs(match):
            img_tag = match.group(0)
            # Try to extract width/height from style attribute
            ...
            if style_match:
                ...
                img_tag = re.sub(r'\s+style="[^"]*"', '', img_tag, flags=re.IGNORECASE)

                # Add height/width as proper attributes if found
                if height_match and 'height=' not in img_tag.lower():
                    height = height_match.group(1)
                    img_tag = img_tag.replace('<img', f'<img height="{height}"')
                if width_match and 'width=' not in img_tag.lower():
                    width = width_match.group(1)
                    img_tag = img_tag.replace('<img', f'<img width="{width}"')

            return img_tag

        # Apply the replacement to extract dimensions and remove style attributes
        html = re.sub(r'<img[^>]*>', replace_style_with_attrs, html, flags=re.IGNORECASE)

        # Also remove style attributes from other elements (para, div, etc.) as fallback
    ...
    def _process_markdown_content(self, content: str) -> str:
        ...
        # Sanitize content before processing (remove unsupported HTML attributes)
        content = self._sanitize_html_for_pdf(content)
        ...
        # Sanitize the generated HTML as well
